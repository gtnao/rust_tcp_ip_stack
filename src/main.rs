...
use anyhow::Result;
use rust_tcp_ip_stack::net::{LoopbackNetDevice, NetDeviceContext, NetDeviceType};
...
fn main() -> Result<()> {
    ...
    net_device_context.register(NetDeviceType::Loopback(LoopbackNetDevice::new()))?;
    ...
    loop {
        net_device_context.transmit(0, "hello".to_string())?;
        thread::sleep(Duration::from_secs(1));
    }
}
